...
ordersApp.controller('OrderListController', function($scope, $http) {
    $http.get('api/zakaz/getall').success(function(data, status, headers, config) {
        $scope.orders = data;
    });
    ...
    $scope.addOrder = function () {
        if ($scope.newOrder.validate()) {
            $http.post('api/zakaz/add', $scope.newOrder).success(function(data) {
    ...
    $scope.deleteOrder = function (id, index) {
        $http.post('api/zakaz/delete/' + id).success(function (data) {
